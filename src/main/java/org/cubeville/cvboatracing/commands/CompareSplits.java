...
public class CompareSplits extends Command {
	...
	public CommandResponse execute(Player player, Set<String> set, Map<String, Object> map, List<Object> baseParameters)
		...
		for (int splitIndex : s1Splits.keySet()) {
			...
			result.addMessage(
				"§6§lCP" + splitIndex + ":§f " + BoatRaceUtilities.formatTimeString(s1Time) + " " +
					comparison + "§f " +
					BoatRaceUtilities.formatTimeString(s2Time)
			);
		}

		result.addMessage(
			"§6§lEND:§f " + BoatRaceUtilities.formatTimeString(s1.getFinalTime()) + " " +
				getComparison(s1.getFinalTime(), s2.getFinalTime()) + "§f " +
				BoatRaceUtilities.formatTimeString(s2.getFinalTime())
		);
	...
	private String getComparison(long t1, long t2) {
		if (t1 > t2) {
			return "§c+" + BoatRaceUtilities.formatTimeString(t1 - t2);
		} else if (t1 < t2) {
